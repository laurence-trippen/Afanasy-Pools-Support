...
class Dialog(QtWidgets.QWidget):
    def __init__(self):
        ...
        # Render Pools
        lPools = QtWidgets.QHBoxLayout()
        joblayout.addLayout(lPools)
        lPools.addWidget(QtWidgets.QLabel('Render Pool:', self))
        self.cbSelectedPool = QtWidgets.QComboBox(self)
        lPools.addWidget(self.cbSelectedPool)
        self.fill_pool_combobox()
    ...
    # Pools Support Addition
    def fill_pool_combobox(self):
        self.cbSelectedPool.addItem("Don't use pool.")
        ...
        af.MongoDB.connection.connect("mongodb://" + config["host"] + ":" + str(config["port"]))
        self.pools = af.MongoDB.connection.findAllPools()
        for pool in self.pools:
            self.cbSelectedPool.addItem(pool["name"])
